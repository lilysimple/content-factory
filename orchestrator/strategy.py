...
def archive(chat_id: int) -> tuple[list[str], list[str]]:
    """Недавние темы и невышедшее. Публичная точка входа.

    Нужна тем, кто планирует не через эту роль. Первый потребитель — мост:
    без неё субагент шарил `Glob` по `posts/` и `plans/` папки бренда, то
    есть вычислял из файлов то, что база знает точно. Ровно та ошибка, за
    которую ловили Director на пересчёте свободных слотов: файл в папке
    остаётся после того, как тему сняли, а строка в `themes` меняет статус.

    Возвращает недавние темы и невышедшее — оба списка готовыми строками.
    """
    return _archive(chat_id), _leftovers(chat_id)
