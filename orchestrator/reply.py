...
from orchestrator import agent, bridge
...
def _state(chat_id: int) -> str:
    ...
    lines = [
        ...
        # Список собирается из кода, а не пишется словами. Прежняя вшитая
        # строка успела соврать дважды: обещала, что сценарии Reels «ещё
        # не подключены», когда Редактор Reels работал с 15.08, и не знала
        # про Идеатора. Это самый повторяющийся баг проекта — фраза о
        # состоянии переживает состояние, которое описывала.
        "Что завод умеет сделать по просьбе: "
        + "; ".join(bridge.WORKFLOWS.values()) + ".",
        "Такую просьбу словами разбирает команда в Claude Code: она сама "
        "решает, кого из ролей звать. Публикация идёт мимо неё, кодом.",
