...
def profile(b, sections: tuple[str, ...], limit: int = PROFILE_LIMIT,
    ...
    `files` — целые файлы профиля поверх секций, ключами из
    `brand_store.PROFILE`. Нужны тем, кто пишет и произносит: механика
    хука живёт в `hooks.md`, а не в ЯДРЕ, потому что меняется она по
    замерам площадки, а ЯДРО по замерам не переписывают.

    Файла нет — блок так и говорит. Молчание здесь читалось бы как
    «правил нет», и роль сочиняла бы их заново на каждом ролике.
    """
    parts = [s for s in (b.section("core", n) for n in sections) if s]
    head = "\n\n".join(parts) or b.read("core")

    # У приложенных файлов бюджет свой, поверх `limit`, а не внутри него.
    # Общий потолок означал бы, что один источник молча вытесняет другой:
    # на `lily-space` секции ЯДРА занимают 8 598 знаков, и `hooks.md`,
    # дописанный в хвост, либо не уезжал вовсе, либо срезал «Формат»
    # вместе с разметкой Telegram. Оба исхода невидимы на глаз.
    blocks = []
    if len(head) > limit:
        blocks.append(f"## Профиль обрезан\n\nСекции ЯДРА не поместились в "
                      f"{limit} знаков ({len(head)}). Хвост последней секции "
                      f"до тебя не доехал: чего в промпте нет, того у тебя "
                      f"нет, и достраивать его нельзя.")
        head = head[:limit]
