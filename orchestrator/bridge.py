...
def running() -> str:
    ...
    Читатели у этого ответа только показывающие: «что сейчас идёт» в
    `/queue` и строка человеку. Решение «пускать ли следующую» принимает
    `free_lane`: с полосами «занято» перестало быть одним битом, и один
    бит на этот вопрос больше не отвечает.
...
# ── полосы ────────────────────────────────────────────────────────────
#
# Мост держал один процесс Claude Code за раз. Это был потолок, за
# которым стояли две разные причины, и разделить их стоило двух строк:
#
# 1. **Железо.** Каждый прогон это headless Claude Code, иногда с Chrome
#    для рендера. Три таких разом на ноутбуке идут медленнее, чем три
#    подряд, — и человек ждёт дольше, а не меньше.
# 2. **Общие данные.** Два плана, идущие разом, видят одни и те же
#    свободные слоты и оба в них садятся. Ровно от этого очередь хранит
#    просьбу, а не готовую задачу: `input.md` собирается в момент, когда
#    очередь дошла.
#
# Вторая причина снимается не потолком, а полосой: задачи **одного
# рода** по-прежнему идут по очереди, а разные — вместе. Сводка недели и
# монтаж не спорят ни за слот, ни за тему, и ждать друг друга им незачем.
#
# Полоса это workflow. Не «роль» и не «чат»: спорят за данные именно
# работы одного рода, а два чата это два тенанта с разными базами тем.

# Сколько прогонов идут разом. Два, а не больше: третий на этом железе
# отнимает у первых двух больше, чем приносит сам. Замер — `NOTES.md`.
LANES = 2


def running_rows() -> list[Any]:
    """Прогоны, которые идут прямо сейчас.

    Строка старше `TIMEOUT` живой быть не может: `run` убивает процесс по
    этому потолку и сам проставляет исход. Значит, такая строка осталась
    от процесса, которого больше нет, — launchd перезапустил завод, или
    инстанс сняли через `kill -9`.
    """
    return db.q(
        "SELECT task_id, workflow, chat_id FROM bridge_runs "
        "WHERE status = 'running' AND started_at > datetime('now', ?) "
        "ORDER BY started_at DESC", f"-{TIMEOUT} seconds")


def busy_lanes() -> set[str]:
    """Роды работ, которые сейчас заняты.

    Считаются по двум спискам, и второй тут не перестраховка. Строка
    очереди становится `taken` в `take`, а строка прогона появляется
    позже — внутри `_serve`, когда задача уже собрана на диске. В эту
    щель насос успевает спросить снова, увидеть полосу свободной и взять
    вторую работу того же рода: ровно то, от чего полоса и заведена.
    """
    lanes = {str(r["workflow"] or _lane_of(r["task_id"]))
             for r in running_rows()}
    lanes |= {str(r["workflow"]) for r in db.q(
        "SELECT workflow FROM bridge_queue WHERE status = 'taken'")}
    return {x for x in lanes if x}


def _lane_of(task_id: str) -> str:
    """Полоса по id прогона: `ГГГГ-ММ-ДД-workflow-NN`.

    Запасной путь для строк, у которых `workflow` пустой: журнал старше
    этой колонки, и старая строка не должна молча занимать чужую полосу
    или, наоборот, освобождать свою.
    """
    parts = str(task_id or "").split("-")
    return parts[3] if len(parts) >= 5 else ""


def free_lane(workflow: str) -> bool:
    """Можно ли пускать работу этого рода прямо сейчас."""
    lanes = busy_lanes()
    return len(lanes) < LANES and workflow not in lanes


def starts_now(qid: int, workflow: str) -> bool:
    """Начнётся ли эта строка сразу, а не после кого-то.

    Свободной полосы мало: в той же полосе может ждать строка, которую
    насос ещё не взял. Сказать такой задаче «начинаю работу» значит
    обещать человеку минуты, которых не будет.
    """
    return free_lane(workflow) and ahead_in_lane(qid, workflow) == 0


def ahead_in_lane(qid: int, workflow: str) -> int:
    """Сколько работ того же рода стоит перед этой.

    Место в общей очереди с полосами перестало отвечать на вопрос «когда
    до меня дойдёт»: сводка, вставшая третьей за двумя планами, начнётся
    первой. Человеку важна своя полоса, а не длина всей очереди.
    """
    row = db.one("SELECT count(*) AS n FROM bridge_queue "
                 "WHERE status = 'waiting' AND workflow = ? AND id < ?",
                 workflow, qid)
    return int(row["n"]) + (1 if workflow in busy_lanes() else 0)
...
def take() -> Any:
    ...
    if len(lanes) >= LANES:
        return None
    with db.tx() as c:
        # Не «первая строка», а «первая, чья полоса свободна»: иначе
        # план, ждущий занятой полосы, держит за собой сводку и монтаж,
        # которым ждать нечего. Это и есть разница между очередью из
        # одной полосы и очередью из нескольких.
        row = None
        for cand in c.execute("SELECT * FROM bridge_queue "
                              "WHERE status = 'waiting' ORDER BY id"):
            if str(cand["workflow"]) not in lanes:
                row = cand
                break
        if row is None:
            return None
...
# Workflow, где тема — обязательный вход и её может не оказаться в
# плане. Плану и сводке этот блок не нужен: план темы заводит сам,
# Ресёрчер тем не берёт вовсе.
ADHOC_WORKFLOWS = ("post", "reels", "design")


def _adhoc(task_id: str) -> list[str]:
    """Как завести тему, которой в плане нет.

    Просьба «сделай пост по такой теме» приходит мимо плана регулярно, а
    посадка требует тему в базе: `editor.land` и `design.land` отказывают
    словами «темы нет в базе», и готовый текст остаётся в `tasks/`, то
    есть в `.gitignore`. Придумать `theme_id` нельзя — его проверяют.

    Команда, а не поле в контракте: тема нужна **до** работы ролей.
    Редактор и Дизайнер в одном прогоне идут по одному id, а посадка
    случается после конца прогона — id, заведённый там, второму уже не
    достался бы.
    """
    return ["", "## Темы под запрос нет в плане", "",
            "Тема заводится по факту просьбы — командой, до вызова ролей. "
            "Печатает один id, его и ставь в `theme_id` контрактов:", "",
            "```bash", f"./.venv/bin/python tools/theme_adhoc.py {task_id} \\",
            "    --plat instagram --format карусель \\",
            '    --title "рабочий заголовок" --hook "хук одной строкой"',
            "```", "",
            "Площадка из набора: telegram, instagram, youtube. Дата у "
            "такой темы остаётся пустой — слот в плане она не занимает.",
            "Заводи её только когда в таблице выше подходящей нет: "
            "лишняя тема вне плана это мусор в базе, который увидит человек.",
            "Что завёл — назови строкой в `final.md`."]
...
def create_task(chat_id: int, ask: str, *, workflow: str, today: str,
    ...
    for build in CONTEXT.get(workflow, (_nothing,)):
        lines += build(chat_id)
    if workflow in ADHOC_WORKFLOWS:
        lines += _adhoc(task_id)
