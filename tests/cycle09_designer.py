...
async def main() -> None:
    ...
    # Роль вправе взять заголовок вместо хука, но узнаёт об этом человек
    # от кода, а не от её доброй воли: обложка, разошедшаяся с первой
    # фразой ролика, молча выглядит как задуманная.
    check("хук на обложке узнаётся",
          design.cover_source(reel, {"line1": "Сначала", "line2": "задай мне",
                                     "line3": "несколько вопросов"}) == "hook")
    check("заголовок вместо хука узнаётся",
          design.cover_source(reel, {"line1": "Пусть модель",
                                     "line2": "соберёт с вас",
                                     "line3": "контекст"}) == "title")
    said: list[str] = []
    design._say_source(reel, [{"slots": {"line1": "Пусть модель",
                                         "line2": "соберёт контекст"}}], said)
    check("подмена названа строкой", any("заголовок темы" in x for x in said),
          str(said))
    said = []
    design._say_source(reel, [{"slots": {"line1": "Сначала задай",
                                         "line2": "мне вопросов"}}], said)
    check("на хуке лишнего не говорится", said == [], str(said))
