...
# Имена продуктов и собственные понятия завода. Последние пять — падежи
# слова ЯДРО: так называется файл профиля бренда и карточка распаковки.
# Без них написать пост про собственный инструмент было нельзя вовсе:
# скрипт давал отказ, Редактор переписывал дважды и тема оставалась
# `idea`. Тема на 18.08 в плане была именно такая.
CAPS_OK = {"CLAUDE", "CHATGPT", "OPENAI", "TELEGRAM", "YOUTUBE", "LINKEDIN",
           "INSTAGRAM", "REELS", "SHORTS", "SEO", "CRM", "PDF", "HTML",
           "ЯДРО", "ЯДРА", "ЯДРУ", "ЯДРОМ", "ЯДРЕ"}
