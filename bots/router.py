"""Маршрутизация: кому из ролей уходит сообщение.

Три сигнала по приоритету: явное упоминание → топик → смысл.
При неоднозначности не угадываем, а спрашиваем одной кнопкой.
"""
from __future__ import annotations

import re
from dataclasses import dataclass

# Топик задаёт роль по умолчанию: написанное в «Дизайн» идёт дизайнеру.
TOPIC_DEFAULT: dict[str, str] = {
    "research": "research",
    "strategy": "strategy",
    "review":   "editor",
    "reels":    "reels",
    "design":   "design",
    "photos":   "design",
    "queue":    "publisher",
    "metrics":  "research",
}

TRIGGERS: dict[str, tuple[str, ...]] = {
    "research": (
        "сводк", "дайджест", "что нового", "тренд", "новости ниши",
        "конкурент", "разбери рилс", "статистик", "метрик", "что зашло",
        # Фактура под тему: тот же Ресёрчер, другой его продукт.
        "фактур", "чем подкрепить", "подпереть тему",
    ),
    "strategy": (
        "план", "контент-план", "тем", "что постить", "спланируй",
        "идеи постов", "рубрик", "стратеги",
    ),
    "editor": (
        "напиши пост", "текст", "перепиши", "поправь", "сделай короче",
        "адаптируй", "анонс", "описание для youtube",
    ),
    "reels": (
        "сценари", "рилс", "reels", "shorts", "суфл", "сними ролик", "хук",
    ),
    "design": (
        "обложк", "карусел", "превью", "оформи", "карточк", "визуал",
        "сверстай",
    ),
    "montage": (
        "смонтируй", "смонтировать", "собери ролик", "собери видео",
        "готово видео", "сделай монтаж", "монтаж готов",
        "нареж", "нареза", "разрежь", "разбей запись", "на рилсы",
        # Просьба про несколько роликов из одной записи. Словá берутся
        # предметные: «из этого видео» ловило бы и «напиши пост из этого
        # видео», где работа как раз не монтажная.
        "несколько рилс", "несколько ролик", "пару рилс", "нарезк", "клипы",
        "субтитр",
        # Сплит просят и без слова «смонтируй»: «сделай сплит», «собери
        # с панелью». Какую работу это значит, решает `montage`.
        "сплит", "с панелью", "инфографик",
    ),
    "publisher": (
        "опубликуй", "в очередь", "перенеси на", "когда выходит",
        "отмени публикацию", "расписани",
    ),
    "assistant": (
        "что в очереди", "статус", "что вышло", "найди пост", "покажи ядро",
        "стоп-слов", "поменяй правило", "настрой", "выгрузи",
    ),
}

# Коллизии, разведённые явно: проверяются до общего словаря.
OVERRIDES: tuple[tuple[str, str], ...] = (
    ("когда выходит", "publisher"),
    ("что там с",     "assistant"),
    ("покажи ядро",   "assistant"),
    # «поправь субтитры» весит у Редактора и у монтажа одинаково, а
    # ничья разводится порядком словаря — то есть в пользу Редактора.
    ("субтитр",       "montage"),
)

# Ссылка на запись в топике Reels — это всегда материал на монтаж, а не
# просьба к Редактору Reels. Слово «рилс» весит на сценариста, поэтому
# «сделай из этого видео несколько рилс <ссылка>» уходило писать суфлёр
# по теме, которой нет, вместо того чтобы резать присланную запись.
# Разводим не словарём, а фактом: в сообщении есть ссылка на видео.
VIDEO_HOSTS = ("youtube.com", "youtu.be", "vimeo.com", "rutube.ru")
VIDEO_LINK = re.compile(
    r"https?://\S*(?:" + "|".join(h.replace(".", r"\.") for h in VIDEO_HOSTS)
    + r")\S*", re.I)

# Фразы, которые честно неоднозначны — спрашиваем, а не гадаем.
AMBIGUOUS = re.compile(r"сделай (что-нибудь|материал|контент)", re.I)


def is_footage(text: str, topic_key: str | None) -> bool:
    """Ссылка на запись в топике Reels — материал на монтаж.

    Отдельной функцией, а не строчкой внутри `resolve`, потому что тот же
    факт нужен раньше маршрутизации: `refresh.wants_refresh` срабатывает
    на **любую** ссылку и стоит в `handlers` выше, поэтому присланный в
    Reels дубль уезжал перечитывать профиль и до монтажа не доходил
    вовсе. Знание одно, место одно.
    """
    return topic_key == "reels" and bool(VIDEO_LINK.search(text or ""))


@dataclass(frozen=True)
class Route:
    role: str | None            # None → надо спросить
    reason: str                 # mention | topic | intent | ambiguous


def resolve(text: str, topic_key: str | None, usernames: dict[str, str]) -> Route:
    low = (text or "").lower()

    # 1. Явное упоминание бота выигрывает у всего.
    for role, username in usernames.items():
        if username and f"@{username.lower()}" in low:
            return Route(role, "mention")

    # 2. Явные коллизии.
    for phrase, role in OVERRIDES:
        if phrase in low:
            return Route(role, "intent")

    # 3. Ссылка на запись в топике Reels: материал, а не тема.
    if is_footage(low, topic_key):
        return Route("montage", "intent")

    if AMBIGUOUS.search(low):
        return Route(None, "ambiguous")

    # 4. Смысл: вес роли это суммарная длина совпавших триггеров.
    #
    # Считать штуки нельзя: короткий триггер ловит подстроку в чужом
    # слове и весит столько же, сколько точное попадание. «Напиши пост
    # по теме 2026-09-01-telegram-01» давало ничью 1:1 — `напиши пост`
    # у Редактора против `тем` внутри слова «теме» у Стратега, — и
    # `max` разводил её порядком ключей в словаре, то есть в пользу
    # Стратега. Человек просил текст, а завод собирал план недели:
    # десять минут работы не по адресу и занятый мост.
    #
    # Длина это грубая мера точности совпадения, и её хватает: у пары
    # выше счёт становится 11 против 3. Ничья остаётся возможной, и
    # разводится она по-прежнему порядком `TRIGGERS` — но теперь это
    # редкий случай, а не каждая фраза со словом «тема».
    hits = {
        role: sum(len(t) for t in triggers if t in low)
        for role, triggers in TRIGGERS.items()
    }
    best = max(hits, key=lambda r: hits[r])
    if hits[best] > 0:
        return Route(best, "intent")

    # 5. Топик как контекст по умолчанию.
    if topic_key and topic_key in TOPIC_DEFAULT:
        return Route(TOPIC_DEFAULT[topic_key], "topic")

    # 6. Ничего не сработало — разбирается Ассистент.
    return Route("assistant", "intent")
