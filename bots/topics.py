...
и создаёт топики сам. Список пополняется: топик, добавленный сюда после
того, как группа уже собрана, заводится при первой же отправке в него —
`Registry._thread`, — а не ждёт, пока человек скажет «готово».
"""
